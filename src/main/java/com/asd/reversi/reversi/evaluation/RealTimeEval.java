...
public class RealTimeEval implements Evaluator {
        ...
        public int eval(int[][] board, int player) {
            ...
            int[] weights = weightSetForDiscCount[Helper.getTotalStoneCount(board)];
        ...
        public RealTimeEval(int[][] weightSet, int[] timingSet) {
        ...
        public static int pieces(int[][] board, int player) {
            ...
            int mySC = Helper.getPlayerStoneCount(board, player);
            int opSC = Helper.getPlayerStoneCount(board, oplayer);
        ...
        public static int stability(int[][] board, int player) {
            ...
            if (board[0][0] == player) myS += Helper.getStableDisks(board, player, 0, 0).size();
            if (board[0][7] == player) myS += Helper.getStableDisks(board, player, 0, 7).size();
            if (board[7][0] == player) myS += Helper.getStableDisks(board, player, 7, 0).size();
            if (board[7][7] == player) myS += Helper.getStableDisks(board, player, 7, 7).size();

            if (board[0][0] == oplayer) opS += Helper.getStableDisks(board, oplayer, 0, 0).size();
            if (board[0][7] == oplayer) opS += Helper.getStableDisks(board, oplayer, 0, 7).size();
            if (board[7][0] == oplayer) opS += Helper.getStableDisks(board, oplayer, 7, 0).size();
            if (board[7][7] == oplayer) opS += Helper.getStableDisks(board, oplayer, 7, 7).size();
        ...
        public static int mobility(int[][] board, int player) {
            ...
            int myMoveCount = Helper.getMovesAsDetails(board, player).size();
            int opMoveCount = Helper.getMovesAsDetails(board, oplayer).size();
        ...
        public static int frontier(int[][] board, int player) {
            ...
            int myF = Helper.getFrontierSquares(board, player).size();
            int opF = Helper.getFrontierSquares(board, oplayer).size();

            return 100 * (myF - opF) / (myF + opF + 1);
        }

        public static int cornerGrab(int[][] board, int player) {
            ArrayList<MoveDetails> moves = Helper.getMovesAsDetails(board, player);

            for (MoveDetails m : moves) {
                //if player have corner move return 1
                if (m.getX() == 0 && m.getY() == 0) return 100;
                if (m.getX() == 7 && m.getY() == 0) return 100;
                if (m.getX() == 0 && m.getY() == 7) return 100;
                if (m.getX() == 7 && m.getY() == 7) return 100;
            }
